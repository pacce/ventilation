...
RC_GTEST_PROP(Comparison, Greaterf32, (const V32& xs)) {
    V32 ys(1.0);
    RC_ASSERT((xs + ys) >  xs);
    RC_ASSERT((xs + ys) >= xs);
}

RC_GTEST_PROP(Comparison, Lesserf32, (const V32& xs)) {
    V32 ys(1.0);
    RC_ASSERT(xs <  (xs + ys));
    RC_ASSERT(xs <= (xs + ys));
}
...
RC_GTEST_PROP(Comparison, Greaterf64, (const V64& xs)) {
    V64 ys(1.0);
    RC_ASSERT((xs + ys) >  xs);
    RC_ASSERT((xs + ys) >= xs);
}

RC_GTEST_PROP(Comparison, Lesserf64, (const V64& xs)) {
    V64 ys(1.0);
    RC_ASSERT(xs <  (xs + ys));
    RC_ASSERT(xs <= (xs + ys));
}
