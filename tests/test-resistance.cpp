...
RC_GTEST_PROP(Comparison, Greaterf32, (const R32& xs)) {
    R32 ys(1.0f);
    RC_ASSERT((xs + ys) > xs);
    RC_ASSERT((xs + ys) >= xs);
}

RC_GTEST_PROP(Comparison, Lesserf32, (const R32& xs)) {
    R32 ys(1.0f);
    RC_ASSERT(xs <  (xs + ys));
    RC_ASSERT(xs <= (xs + ys));
}
...
RC_GTEST_PROP(Comparison, Greaterf64, (const R64& xs)) {
    R64 ys(1.0);
    RC_ASSERT((xs + ys) > xs);
    RC_ASSERT((xs + ys) >= xs);
}

RC_GTEST_PROP(Comparison, Lesserf64, (const R64& xs)) {
    R64 ys(1.0);
    RC_ASSERT(xs <  (xs + ys));
    RC_ASSERT(xs <= (xs + ys));
}
