...
using P32 = ventilation::pressure::Delta<float>;
using P64 = ventilation::pressure::Delta<double>;
...
RC_GTEST_PROP(Comparison, Greaterf32, (const P32& xs)) {
    P32 ys(1.0f);
    RC_ASSERT((xs + ys) > xs);
    RC_ASSERT((xs + ys) >= xs);
}

RC_GTEST_PROP(Comparison, Lesserf32, (const P32& xs)) {
    P32 ys(1.0f);
    RC_ASSERT(xs <  (xs + ys));
    RC_ASSERT(xs <= (xs + ys));
}
...
RC_GTEST_PROP(Comparison, Greaterf64, (const P64& xs)) {
    P64 ys(1.0);
    RC_ASSERT((xs + ys) > xs);
    RC_ASSERT((xs + ys) >= xs);
}

RC_GTEST_PROP(Comparison, Lesserf64, (const P64& xs)) {
    P64 ys(1.0);
    RC_ASSERT(xs <  (xs + ys));
    RC_ASSERT(xs <= (xs + ys));
}
